...
public:
  ...
  explicit ScanWorker(ScanType type, const QStringList &customPaths,
...
class EDRBridge : public QObject {
  Q_OBJECT
  Q_PROPERTY(bool protectionActive READ isProtectionActive NOTIFY
...
public slots:
  ...
  // Scanning
  void startQuickScan();
  void startCustomScan(const QStringList &paths);
  void startFullScan();
  void cancelScan();
  void pauseScan();
  void resumeScan();
...
private slots:
  ...
  void onSharedStatusUpdated(bool protectionActive, int activeIncidents,
                             int totalIncidents, int totalEvents,
                             int highestRisk, bool procMon, bool fileMon,
                             bool netMon, bool regMon);
