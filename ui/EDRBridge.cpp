...
ScanWorker::ScanWorker(ScanType type, const QStringList &customPaths,
                       QObject *parent)
    : QObject(parent), type_(type), customPaths_(customPaths) {}
...
void ScanWorker::doScan() {
  ...
  if (type_ == Quick) {
    if (!customPaths_.isEmpty()) {
      // Custom scan: user-selected folder(s)
      scanPaths = customPaths_;
    } else {
...
void ScanWorker::scanDirectory(const QString &path, int &scanned,
                               int totalEstimate) {
  ...
  while (it.hasNext() && !cancelled_) {
    ...
    if (scanned % 50 == 0) {
      ...
      if (scanned > 100) {
        ...
        int remaining =
            static_cast<int>((totalEstimate - scanned) * msPerFile / 1000);
        int mins = remaining / 60;
        int secs = remaining % 60;
        emit estimatedTimeChanged(QString("%1:%2")
...
bool ScanWorker::isSuspiciousFile(const QString &filePath,
                                  QString &threatName) {
  ...
  for (const auto &name : suspiciousNames) {
    if (baseName.contains(name)) {
      threatName =
          "Heuristic.Suspicious." + name.mid(0, 1).toUpper() + name.mid(1);
      return true;
    }
  }
...
EDRBridge::EDRBridge(QObject *parent) : QObject(parent) {
...
EDRBridge::~EDRBridge() {
  ...
  if (ipcThread_ && ipcThread_->isRunning()) {
    if (ipcWorker_) {
      QMetaObject::invokeMethod(ipcWorker_, &IPCWorker::stopConnection,
                                Qt::QueuedConnection);
    }
    ipcThread_->quit();
...
void EDRBridge::initializeBackendConnection() {
  ...
  connect(ipcThread_, &QThread::started, ipcWorker_,
          &IPCWorker::startConnection);
  connect(ipcWorker_, &IPCWorker::eventReceived, this,
          &EDRBridge::onPipeEventReceived);
  connect(ipcWorker_, &IPCWorker::statusUpdated, this,
          &EDRBridge::onSharedStatusUpdated);
  connect(ipcWorker_, &IPCWorker::connectionStateChanged, this,
          &EDRBridge::onBackendConnectionChanged);
...
void EDRBridge::startQuickScan() {
  ...
  connect(scanWorker_, &ScanWorker::progressChanged, this,
          &EDRBridge::scanProgressChanged);
  connect(scanWorker_, &ScanWorker::currentFileChanged, this,
          &EDRBridge::scanCurrentFileChanged);
  connect(scanWorker_, &ScanWorker::threatDetected, this,
          [this](const QString &file, const QString &threat) {
  ...
  connect(scanWorker_, &ScanWorker::scanFinished, this,
          &EDRBridge::onScanFinished);
  connect(scanWorker_, &ScanWorker::estimatedTimeChanged, this,
          &EDRBridge::scanEstimatedTimeChanged);
...
void EDRBridge::startCustomScan(const QStringList &paths) {
  if (scanThread_ && scanThread_->isRunning())
    return;

  scanThread_ = new QThread(this);
  scanWorker_ = new ScanWorker(ScanWorker::Quick, paths);
  scanWorker_->moveToThread(scanThread_);
  ...
  addLogEntry("Scan", "Quick scan started: " + paths.join(", "), "", "Info");
  scanThread_->start();
}
...
void EDRBridge::quarantineFile(const QString &filePath) {
  ...
  addLogEntry("Threat", "File quarantined: " + info.fileName(), filePath,
              "Warning");
}

bool EDRBridge::restoreFile(const QString &quarantinePath,
...
bool EDRBridge::deleteFilePermanently(const QString &quarantinePath) {
  ...
  if (!quarantinePath.contains("quarantine", Qt::CaseInsensitive)) {
    addLogEntry("System", "Delete blocked: file not in quarantine directory",
                quarantinePath, "Warning");
    return false;
  }
  ...
  addLogEntry("System", "Quarantined file permanently deleted", quarantinePath,
              "Warning");
  return true;
}
...
void EDRBridge::setScanSensitivity(int level) {
  ...
  addLogEntry("System", QString("Scan sensitivity changed to %1").arg(level),
              "", "Info");
...
void EDRBridge::setAutoScanOnStartup(bool enabled) {
  ...
  addLogEntry("System",
              enabled ? "Auto-scan on startup enabled"
                      : "Auto-scan on startup disabled",
              "", "Info");
  emit settingsChanged();
}
...
void EDRBridge::setHeuristicScanEnabled(bool enabled) {
  ...
  addLogEntry("System",
              enabled ? "Heuristic scanning enabled"
...
void EDRBridge::updateDefinitions() {
  ...
  QTimer::singleShot(2000, this, [this]() {
    addLogEntry("System", "Threat definitions updated successfully", "",
                "Info");
...
void EDRBridge::onPipeEventReceived(const QString &jsonLine) {
  ...
  if (eventType.contains("RISK_THRESHOLD") || riskScore >= 60) {
    ...
    totalThreats_++;
    emit threatCountChanged(totalThreats_);
    emit threatNotification(QString("High Risk: %1 (PID %2, Score %3)")
                                .arg(processName)
                                .arg(pid)
                                .arg(riskScore),
                            processName);
  } else if (eventType.contains("CONTAINMENT") ||
  ...
  QString details =
      QString("[%1] PID=%2 %3").arg(eventType).arg(pid).arg(processName);
  if (riskScore > 0) {
...
void EDRBridge::onSharedStatusUpdated(bool protectionActive,
                                      int activeIncidents, int totalIncidents,
                                      int totalEvents, int highestRisk,
                                      bool procMon, bool fileMon, bool netMon,
                                      bool regMon) {
  (void)totalEvents;
  (void)highestRisk;
...
int EDRBridge::activeIncidentCount() const { return activeIncidents_; }

int EDRBridge::totalIncidentCount() const { return totalIncidents_; }
